...
export async function MongoIndexerExpress(app, options) {
   ...
   * Imports
   */
  const { MongoIndexer } = await import('./mongo-indexer.js');
  const { MongoIndexerRestore } = await import('./mongo-indexer-restore.js');
  const { MongoIndexerBackups } = await import('./mongo-indexer-backups.js');
  const { MongoClient } = await import('../services/mongo-client.js');
  const logger = await (await import('../utils/logger.js')).Logger();

  logger.debug(`MongoIndexerExpress [setup] options`, { ...options });
